...
#Step 2 - import fracking data

```{r}
fracking <- read_csv(here::here("week1", "data", "fracking.csv"))
```

##Step 3 - clean/wrangle fracking data

```{r}
fracking_clean <- fracking |> 
  
  # clean column names ----
  janitor::clean_names() |> # change column names to snake case "_". Better readability of column headers
  
  # clean up dates ----
  mutate(job_start_date = str_remove(job_start_date, " AM")) |> # remove " AM" from string for job_start_date column cells
  mutate(datetime_start = mdy_hms(job_start_date)) |> #create a new column called "datetime_start". Input job_start_date data w/out " AM"
  mutate(year = year(datetime_start)) |> #create a new column called "year". Input daate_time_start data year

  # select relevant cols ----
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) |> #isolate columns listed to refer to in fracking_clean
  
  # remove out non-state names ----
  filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill", "Midland", "Red River", "Roosevelt", "Rusk", "State", "WARD")) |> 
  
  # rename state_name column to sn ----
  rename(sn = state_name) |> 
  
  # make all words title case in sn column----
